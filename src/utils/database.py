...
from src.utils.config_loader import read_config, write_config
...
class Database:
    ...
    def get_user_settings(self, user_id: str):
        ...
        selected_fields_keys = f"USER_{user_id}_SELECTED_FIELDS"
        field_order_key = f"USER_{user_id}_FIELD_ORDER"
        default_count_key = f"USER_{user_id}_DEFAULT_COUNT"

        if selected_fields_keys in config and field_order_key in config:
            return {
                'selected_fields': config[selected_fields_keys].split(',') if config[selected_fields_keys] else [],

...
class TelegramBotManager:
    ...
    async def _show_settings(self, message_or_query, state: FSMContext):
        ...
        if isinstance(message_or_query, CallbackQuery):
            ...
            await message_or_query.message.reply(text, reply_markup=reply_markup, parse_mose="HTML")
    ...
    async def _handle_callback(self, query: CallbackQuery, state: FSMContext):
        ...
        if data == "main_menu":
            ...
            await self._toggle_field(query, data.replace("toggle_field", ""), state)
    ...
    async def _start_parsing_flow_from_keyboard(self, message: Message):
        if self.app_manager.is_running:
            ...
            await message.reply("⚠️ Парсинг уже запущен. Дождитесь завершения или остановите его.",
    ...
    def stop(self):
        try:
            self.is_running = False
            # 🚀 non-blocking stop
            # т.е.  дальнейшая остановка происходит в неблокирующем режиме — то есть метод stop не будет ждать полного
            # завершения работы, чтобы не "замораживать" поток
            if hasattr(self, 'dp') and self.dp and self.dp._loop and not self.dp._loop.is_closed():
                # Есть ли у объекта self атрибут dp (обычно это диспетчер сообщений у Telegram бота).
                # Что атрибут dp не пустой (None или другой "ложный" объект).
                # Что у dp есть свойство _loop, где хранится asyncio-событийный цикл.
                # Что этот событийный цикл не закрыт — то есть ещё жив и может выполнять задачи

                asyncio.run_coroutine_threadsafe(self.dp.stop_polling(), self.dp._loop)
                # Допустим, основной цикл событий self.dp._loop работает в одном потоке (обычно — в том, где запущен бот).
                # А сейчас метод stop вызывается из другого (второго) потока.
                # Просто так вызвать эту корутину из другого потока нельзя, потому что event loop «живёт» в своём потоке
                # и не является потокобезопасным.
                # asyncio.run_coroutine_threadsafe — специальная функция, которая безопасно запускает корутину в event
                # loop из другого потока.


                # stop_polling() — это не простая функция, а корутина, которую нужно запускать в асинхронном режиме
                # внутри цикла событий (event loop)
                # asyncio.run_coroutine_threadsafe позволяет запустить асинхронную задачу в конкретном event loop
                # из другого потока, не блокируя текущий
                # Цикл событий (event loop) — это «машина», которая запускает и контролирует выполнение таких корутин
